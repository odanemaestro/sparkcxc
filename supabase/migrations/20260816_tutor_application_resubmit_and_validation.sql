-- CaribPrep - secure tutor application submission / re-submission
-- Fixes rejected re-application failures caused by applicant UPDATE policies
-- and the status-change protection trigger.
-- Also validates application data at the database boundary.
...
begin
  ...
  if length(v_experience) < 10 or length(v_experience) > 2000 then
    raise exception 'Teaching experience must be between 10 and 2,000 characters.';
  end if;
