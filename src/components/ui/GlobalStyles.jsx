  ...
  useEffect(() => {
    ...
    s.textContent = `
      ...
      *{box-sizing:border-box;}
      ...
      ::-webkit-scrollbar-thumb:hover{background:var(--spark-border-strong);}
      ...
      .lesson-section:hover{box-shadow:${T.shadowSm};border-color:${T.tealLight};}
      ...
      .example-box{background:var(--spark-teal-surface);border-left:3px solid ${T.teal};border-radius:0 10px 10px 0;padding:16px 18px;margin:14px 0;}
      ...
      .quiz-option{width:100%;padding:13px 16px;border-radius:${T.rSm}px;font-size:14px;text-align:left;cursor:pointer;border:1.5px solid ${T.border};background:${T.paper};color:${T.inkSoft};margin-bottom:8px;transition:all .18s ${T.ease};font-family:${FB};display:block;}
      ...
      .calendar-icon-btn{width:34px;height:34px;border:1px solid ${T.border};background:${T.paper};color:${T.ink};border-radius:9px;display:inline-flex;align-items:center;justify-content:center;font-size:16px;transition:all .18s ${T.ease};}
      .calendar-icon-btn:hover{background:${T.muted};border-color:var(--spark-border-strong);transform:translateY(-1px);}
      ...
      .calendar-weekday{padding:10px 8px;font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:700;color:${T.textMuted};background:${T.muted};border-bottom:1px solid ${T.border};}
      .calendar-day{min-height:118px;padding:8px;border-right:1px solid ${T.border};border-bottom:1px solid ${T.border};background:${T.paper};position:relative;transition:background .16s ${T.ease};}
      .calendar-day:nth-child(7n){border-right:none;}
      .calendar-day.is-outside{background:${T.muted};}
      .calendar-day:hover{background:var(--spark-paper-raised);}
      .calendar-day.is-today{background:linear-gradient(180deg,${T.tealLight}55,${T.paper} 70%);}
      ...
      .calendar-event{width:100%;text-align:left;border:0;border-left:3px solid ${T.teal};background:var(--spark-teal-surface);color:${T.ink};border-radius:7px;padding:5px 6px;margin-bottom:5px;cursor:pointer;display:block;overflow:hidden;transition:all .16s ${T.ease};}
      ...
      .calendar-event.completed{border-left-color:${T.teal};background:var(--spark-teal-surface);}
      ...
      .calendar-agenda-empty{padding:26px 12px;text-align:center;border:1px dashed ${T.border};border-radius:12px;background:${T.muted};color:${T.textMuted};font-size:13px;line-height:1.55;}
      .calendar-agenda-event{border:1px solid ${T.border};border-radius:12px;padding:13px;margin-bottom:10px;background:${T.paper};box-shadow:${T.shadowSm};}
      ...
      .calendar-view-switch button.active{background:${T.paper};color:${T.ink};box-shadow:${T.shadowSm};}
      ...
      @media(max-width:700px){.lesson-layout{grid-template-columns:1fr!important;}.lesson-sidebar{display:none!important;}.dash-layout{grid-template-columns:1fr!important;}}
    `;
