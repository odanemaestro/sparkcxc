...
const ProgressBar = ({ value, max, color = T.teal, height = 6, style, className = "" }) => {
  ...
  return (
    <div className={`spark-progress-bar ${className}`.trim()} role="progressbar" aria-valuemin="0" aria-valuemax={safeMax}
      ...
      <div className="spark-progress-bar__fill" style={{width:`${pct}%`,minWidth:safeValue > 0 ? 6 : 0,height:"100%",
        background:`linear-gradient(90deg,${color},${color}CC)`,borderRadius:99,transition:`width .36s ${T.ease}`}}/>
    </div>
  );
};
