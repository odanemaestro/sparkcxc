...
describe("Integrated Science Objective 3.5.6 acceptance audit",()=>{
 ...
 test("renders scratched galvanising as a sacrificial-protection cross-section",()=>{for(const term of ["spark-galvanising-svg","continuous zinc coating","scratch exposes iron","water + oxygen reach scratch","zinc oxidises","nearby iron remains protected while zinc is sacrificed","zinc is more reactive than iron"])expect(explorer).toContain(term);expect(css).toContain(".spark-galvanising-svg");expect(css).toContain(".gv-zinc");expect(css).toContain(".gv-protection-flow");expect(css).toContain(".gv-zinc-corrosion");});
 ...
 test("renders the electroplating cell and current path",()=>{expect(explorer).toContain("spark-electroplating-diagram");expect(explorer).toContain("positive electrode");expect(explorer).toContain("cathode, negative");expect(explorer).toContain("metal ions move through the electrolyte");expect(explorer).toContain("DC");expect(css).toContain(".spark-electroplating-diagram");});
