...
describe("Integrated Science Objective 3.2.1 acceptance audit",()=>{
 ...
 test("covers air-mass classifications",()=>{expect(migration).toContain("maritime tropical");expect(migration).toContain("continental polar");expect(migration).toContain("maritime polar");expect(explorer).toContain('id:"cT",name:"Continental tropical"');});
 ...
 test("covers cold and warm front weather",()=>{expect(migration).toContain("heavy showers, thunderstorms");expect(migration).toContain("longer periods of light or steady rain");expect(explorer).toContain("FrontCrossSection");expect(explorer).toContain("steep uplift");expect(explorer).toContain("warm air rises gradually");expect(explorer).toContain("towering cloud + heavy showers");expect(explorer).toContain("layered cloud + longer steady rain");});
 test("covers occluded and stationary fronts",()=>{expect(migration).toContain("cold front catches up with a warm front");expect(migration).toContain("stationary front");expect(explorer).toContain("Occluded front");expect(explorer).toContain("warm air lifted off ground");expect(explorer).toContain("boundary changes little");});
