...
function RespiratorySystemView(){
  const [focus,setFocus]=useState("pathway");
  const details={
    pathway:["Air pathway","Air passes from the nasal cavity through the pharynx, larynx and trachea, then through bronchi and bronchioles to the alveoli."],
    branching:["Branching airways","The trachea divides into right and left bronchi. These branch repeatedly into smaller bronchioles throughout the lungs."],
    alveoli:["Alveoli","Bronchioles end in alveoli. Their thin, moist walls and dense capillary supply support rapid gaseous exchange."],
    diaphragm:["Diaphragm","The diaphragm changes thoracic volume during ventilation and helps move air into and out of the lungs."],
  };
  const selected=details[focus];
  return <div className="spark-respiratory-system">
    <figure className="spark-respiratory-reference">
      <img
        src="https://commons.wikimedia.org/wiki/Special:Redirect/file/Respiratory_system_complete_no_labels.svg"
        alt="Unlabeled scientific diagram of the complete human respiratory system"
        loading="lazy"
      />
      <figcaption>
        <span>Human respiratory system</span>
        <small>
          Reference: <a href="https://commons.wikimedia.org/wiki/File:Respiratory_system_complete_no_labels.svg" target="_blank" rel="noreferrer">Bibi Saint-Pol / Jmarchn</a>
          {" · "}CC BY-SA 3.0
        </small>
      </figcaption>
    </figure>
    <div className="spark-respiratory-reference-focus">
      ...
      <p className="spark-respiratory-pathway">air pathway: nasal cavity → pharynx → larynx → trachea → bronchi → bronchioles → alveoli</p>
    </div>
  </div>;
}
...
export default function GaseousExchangeExplorer(){
  ...
  return <section className="spark-gaseous-exchange">
    ...
    <div className="spark-gas-stage">
      ...
      {view==="respiratory"&&<RespiratorySystemView/>}
