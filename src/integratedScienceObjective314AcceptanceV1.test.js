...
describe("Integrated Science Objective 3.1.4 acceptance audit",()=>{
 ...
 test("covers Moon light and phase cycle",()=>{expect(migration).toContain("reflects sunlight");expect(migration).toContain("29½ days");expect(explorer).toContain("Moon phases");expect(explorer).toContain("spark-moon-phase-svg");expect(explorer).toContain("Sun–Earth–Moon geometry");expect(explorer).toContain("View from Earth");expect(explorer).toContain("The half of the Moon facing the Sun is always illuminated.");expect(explorer).toContain("First quarter");expect(explorer).toContain("Last quarter");});
 ...
 test("responsive and dark-mode ready",()=>{expect(css).toContain("@media(max-width:900px)");expect(css).toContain('html[data-theme="dark"]');expect(css).toContain("SPARK_MOON_PHASE_GEOMETRY_V2");expect(css).toContain(".spark-moon-phase-svg");});
