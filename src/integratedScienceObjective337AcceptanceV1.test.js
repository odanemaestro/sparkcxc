...
describe("Integrated Science Objective 3.3.7 acceptance audit",()=>{
 ...
 test("covers upthrust and Archimedes",()=>{expect(migration).toContain("Upthrust is the upward force");expect(migration).toContain("Archimedes'' principle");expect(migration).toContain("5.0 - 3.0 = 2.0 N");expect(explorer).toContain("Archimedes");expect(explorer).toContain("spark-archimedes-diagram");expect(explorer).toContain("spring balance");expect(explorer).toContain("weight of displaced water");expect(explorer).toContain("this difference is the upthrust");expect(explorer).toContain("fa-force up");expect(explorer).toContain("fa-force down");});
 ...
 test("responsive and dark-mode ready",()=>{expect(css).toContain("@media(max-width:900px)");expect(css).toContain('html[data-theme="dark"]');expect(css).toContain("SPARK_ARCHIMEDES_FORCE_MODEL_V2");expect(css).toContain(".spark-archimedes-diagram");});
