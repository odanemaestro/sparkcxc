...
function InstrumentLab() {
  ...
  return <LabFrame title="Instrument Explorer" intro="Change the instrument controls and read the moving scale. The visual position, highlighted division and corrected reading all respond to the selected settings.">
    ...
    <div className="pm-controls-grid"><Slider id="inst-main" label={mode==='vernier'?'Main scale before vernier zero':'Sleeve reading'} value={main} min={4} max={25} step={1} onChange={setMain} display={mode==='vernier'?`${main} mm`:`${(main/2).toFixed(1)} mm`} /><Slider id="inst-div" label={mode==='vernier'?'Coincident vernier division':'Thimble division'} value={division} min={0} max={mode==='vernier'?9:49} step={1} onChange={setDivision} display={division}/><Slider id="inst-zero" label="Positive zero error" value={zero} min={0} max={.5} step={.1} onChange={setZero} display={mode==='vernier'?`${zero.toFixed(1)} mm`:`${(zero/10).toFixed(2)} mm`} /></div>
    {mode==='vernier' ? (
...
function VectorResultantLab() {
  ...
  return <LabFrame title="Resultant Vector Lab" intro="Build the vector construction and compare it with the calculated resultant. Oblique vectors belong in scale-diagram work; perpendicular vectors can also be calculated directly.">
    ...
    <Plot label="Vector addition construction"><defs><marker id="pm-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerUnits="userSpaceOnUse" markerWidth="8" markerHeight="8" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" className="pm-arrow-fill"/></marker></defs>
...
function ComponentLab() {
  ...
  return <LabFrame title="Component Resolver" intro="A single vector can be replaced by two perpendicular components. Which component uses sine or cosine depends on where the angle is measured.">
    ...
    <Plot label="Vector resolved into horizontal and vertical components"><defs><marker id="pm-arrow2" viewBox="0 0 10 10" refX="8" refY="5" markerUnits="userSpaceOnUse" markerWidth="8" markerHeight="8" orient="auto"><path d="M0 0L10 5L0 10z" className="pm-arrow-fill"/></marker></defs><line x1={ox} y1={oy} x2={hx} y2={oy} className="pm-vector a" markerEnd="url(#pm-arrow2)"/><line x1={hx} y1={oy} x2={hx} y2={hy} className="pm-vector b" markerEnd="url(#pm-arrow2)"/><line x1={ox} y1={oy} x2={hx} y2={hy} className="pm-vector r" markerEnd="url(#pm-arrow2)"/><polyline points={`${ox},${oy} ${hx},${oy} ${hx},${hy}`} className="pm-right-angle"/></Plot>
