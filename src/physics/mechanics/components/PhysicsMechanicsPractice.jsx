...
export default function PhysicsMechanicsPractice({ userId, onBack, onActivity }) {
  ...
  if (mode === "topic") return <main className="physics-mechanics"><div className="pm-shell"><div className="pm-support-head pm-topic-test-head"><div><div className="pm-eyebrow">CSEC Physics practice</div><h1>Topic tests</h1><p>Choose one Mechanics topic and complete a 10-question objective-linked test.</p></div><button type="button" className="pm-btn secondary" onClick={() => setMode("home")}>← Physics practice</button></div><nav className="pm-topics pm-topic-test-topics" aria-label="Physics topic tests">{SECTION_A_TOPICS.map(item => <button key={item.id} type="button" className={`pm-topic-btn ${topicId === item.id ? "active" : ""}`} aria-pressed={topicId === item.id} onClick={() => setTopicId(item.id)}>{item.id} · {item.title}</button>)}</nav><PhysicsTopicQuiz key={topic.id} topic={topic} onActivity={onActivity}/></div></main>;
