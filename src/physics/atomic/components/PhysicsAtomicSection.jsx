...
function LabsView({ topic, progress, setProgress, onActivity, userId }) {
  ...
  return <div className="pm-labs-grid">{labs.map(lab => {
    ...
    const evidence = payload => {
      onActivity?.({ type:'physics_lab_evidence', subject:'CSEC Physics', section:'E', topic:topic.id, lab:lab.id, ...payload });
...
export default function PhysicsAtomicSection({ userId, onBack, onActivity }) {
  const [topicId, setTopicId, mode, setMode] = usePhysicsStudyRoute('E', SECTION_E_TOPICS.map(item => item.id));
