...
export default function PhysicsFormulaList({ onBack }) {
        ...
        <div className="physics-formula-groups">
          {grouped.map(([topic, rows]) => (
            <section key={topic} className="physics-resource-card physics-formula-group">
              <div className="physics-formula-group-head"><strong>{topic}</strong><span>{rows.length} {rows.length === 1 ? 'entry' : 'entries'}</span></div>
