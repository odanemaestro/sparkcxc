...
export default function PhysicsWorkbook({ onBack, onOpenSection, onOpenFormulae }) {
    ...
    <main className="physics-resource-root">
      <div className="physics-resource-shell">
        ...
        <div className="physics-resource-actions">
          {onOpenFormulae && <button type="button" className="physics-resource-primary" onClick={onOpenFormulae}>Open Formula List</button>}
        ...
        <div className="physics-workbook-layout">
          ...
          <article className="physics-workbook-main">
            ...
            {upgrade?.examFocus?.length > 0 && <section className="physics-resource-card"><h3>Exam focus</h3><ul className="physics-resource-list">{upgrade.examFocus.map((item, index) => <li key={index}><MathText prose>{item}</MathText></li>)}</ul>{upgrade.examLanguage && <div className="physics-workbook-exam-language"><strong>Exam language</strong><MathText as="p" prose>{upgrade.examLanguage}</MathText></div>}</section>}

            {formulae.length > 0 && <section className="physics-resource-card"><div className="physics-resource-section-head"><h3>Formulae and relationships</h3>{onOpenFormulae && <button type="button" onClick={onOpenFormulae}>Full list</button>}</div><div className="physics-workbook-formula-grid">{formulae.map((item, index) => <div key={index}><span>{item.name}</span><strong><MathText prose>{item.equation}</MathText></strong><small><MathText prose>{item.unit}</MathText></small></div>)}</div></section>}
            ...
            <section className="physics-resource-card">
              <div className="physics-resource-section-head"><div><h3>Objective by objective</h3><p>{topic.objectiveCount} syllabus objectives in this topic.</p></div><label className="physics-resource-search compact"><span>Find an objective</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search this topic..." /></label></div>
