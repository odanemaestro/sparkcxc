...
export function lensSignedImageDistance({focalLength,objectDistance}){const f=positive(focalLength,'focalLength'),u=positive(objectDistance,'objectDistance');const denom=1/f-1/u;if(Math.abs(denom)<1e-12)return Infinity;return 1/denom;}
export function lensSignedMagnification({imageDistance,objectDistance}){const v=finite(imageDistance,'imageDistance'),u=positive(objectDistance,'objectDistance');if(!Number.isFinite(v))return v>0?-Infinity:Infinity;return -v/u;}
