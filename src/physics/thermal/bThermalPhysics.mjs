...
export function boyleFinalPressure({p1,v1,v2}){return positive(p1,'p1')*positive(v1,'v1')/positive(v2,'v2');}
export function boyleFinalVolume({p1,v1,p2}){return positive(p1,'p1')*positive(v1,'v1')/positive(p2,'p2');}
export function charlesFinalVolume({v1,t1K,t2K}){return positive(v1,'v1')*positive(t2K,'t2K')/positive(t1K,'t1K');}
export function pressureLawFinalPressure({p1,t1K,t2K}){return positive(p1,'p1')*positive(t2K,'t2K')/positive(t1K,'t1K');}
...
export function heatCapacity({massKg,specificHeatJPerKgK}){return positive(massKg,'massKg')*positive(specificHeatJPerKgK,'specificHeatJPerKgK');}
export function sensibleHeat({massKg,specificHeatJPerKgK,deltaTK}){return positive(massKg,'massKg')*positive(specificHeatJPerKgK,'specificHeatJPerKgK')*finite(deltaTK,'deltaTK');}
export function specificHeatFromElectrical({powerW,timeS,massKg,deltaTK}){const d=finite(deltaTK,'deltaTK');if(d===0)throw new RangeError('deltaTK must not be zero');return positive(powerW,'powerW')*positive(timeS,'timeS')/(positive(massKg,'massKg')*d);}
export function latentEnergy({massKg,specificLatentHeatJPerKg}){return positive(massKg,'massKg')*positive(specificLatentHeatJPerKg,'specificLatentHeatJPerKg');}
export function specificLatentHeatFromElectrical({powerW,timeS,massKg}){return positive(powerW,'powerW')*positive(timeS,'timeS')/positive(massKg,'massKg');}
...
export function heatingCurveState({
  energyJ,massKg=1,specificHeatSolid=2100,latentFusion=334000,
  specificHeatLiquid=4200,latentVaporization=2260000,specificHeatVapour=2000,
  startTempC=-20,meltingTempC=0,boilingTempC=100
}){
  let e=nonNegative(energyJ,'energyJ'); const m=positive(massKg,'massKg');
  const cSolid=positive(specificHeatSolid,'specificHeatSolid');
  const cLiquid=positive(specificHeatLiquid,'specificHeatLiquid');
  const cVapour=positive(specificHeatVapour,'specificHeatVapour');
  const qWarmSolid=m*cSolid*(meltingTempC-startTempC);
  if(e<qWarmSolid) return {phase:'solid warming',temperatureC:startTempC+e/(m*cSolid),stageEnergyJ:e};
  ...
  e-=qMelt; const qWarmLiquid=m*cLiquid*(boilingTempC-meltingTempC);
  if(e<qWarmLiquid) return {phase:'liquid warming',temperatureC:meltingTempC+e/(m*cLiquid),stageEnergyJ:e};
