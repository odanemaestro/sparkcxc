...
describe("Integrated Science Objective 3.3.3 acceptance audit",()=>{
 ...
 test("renders the full water cycle as a scientific SVG",()=>{for(const term of ["spark-water-cycle-svg","evaporation","transpiration","evapotranspiration","condensation","precipitation","intercepted by vegetation","surface run-off","infiltration","percolation to groundwater","throughflow through soil","groundwater flow","river flow","sea and ocean storage","lakes and rivers","fresh groundwater"]) expect(explorer).toContain(term);expect(css).toContain(".spark-water-cycle-svg");expect(css).toContain(".wu-groundwater-layer");expect(css).toContain(".wu-cycle-flow");expect(css).toContain(".wu-interception circle");expect(css).toContain(".wu-percolation");expect(css).toContain(".wu-throughflow");});
