...
describe("Integrated Science Objective 3.3.2 acceptance audit",()=>{
 ...
 test("renders the soap-lather comparison as a controlled apparatus SVG",()=>{for(const term of ["spark-hardwater-soaptest-svg","fair soap test","same water volume","same amount of soap","stopper and shake equally","compare lather and scum","water + soap","lather / foam","soap scum","more lather = softer water"]) expect(explorer).toContain(term);expect(css).toContain(".spark-hardwater-soaptest-svg");expect(css).toContain(".hw-foam-bubbles");expect(css).toContain(".hw-scum-particles");});
