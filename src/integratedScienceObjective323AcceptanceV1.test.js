...
describe("Integrated Science Objective 3.2.3 acceptance audit",()=>{
 ...
 test("covers lunar cause and two bulges",()=>{expect(migration).toContain("Moon''s gravity is the main cause");expect(migration).toContain("two broad high-tide bulges");expect(explorer).toContain("Why tides occur");expect(explorer).toContain("near-side bulge");expect(explorer).toContain("far-side bulge");expect(explorer).toContain("low tide");expect(explorer).toContain("Moon's pull differs across Earth");});
 ...
 test("covers tidal timing",()=>{expect(migration).toContain("12 hours 25 minutes");expect(migration).toContain("two high tides");expect(explorer).toContain("Tide timing");expect(explorer).toContain("24 h 50 min");expect(explorer).toContain("Earth rotates through both bulges");});
 ...
 test("renders tsunami generation propagation shoaling and inundation as a scientific SVG",()=>{for(const term of ["spark-tsunami-svg","sudden sea-floor displacement","long wave train travels rapidly across deep water","shallower water slows the wave","water piles up and wave height increases","coastal inundation","rapid sea withdrawal can be a natural warning sign","not part of the regular gravitational tide cycle"]) expect(explorer).toContain(term);expect(css).toContain(".spark-tsunami-svg");expect(css).toContain(".ts-shoaling");expect(css).toContain(".ts-inundation");});
