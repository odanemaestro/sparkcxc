...
describe("Integrated Science Objective 3.4.7 acceptance audit",()=>{
 ...
 test("renders vertebral regions as a labelled column SVG",()=>{for(const term of ["spark-spinal-regions-svg","sr-vertebra cervical","sr-vertebra thoracic","sr-vertebra lumbar","7 vertebrae","12 vertebrae","5 vertebrae","SACRUM","COCCYX","protect the spinal cord"])expect(explorer).toContain(term);expect(css).toContain(".spark-spinal-regions-svg");expect(css).toContain(".sr-vertebra.cervical");expect(css).toContain(".sr-vertebra.thoracic");expect(css).toContain(".sr-vertebra.lumbar");});
