...
describe("Integrated Science Objective 3.7.3 acceptance audit",()=>{
 ...
 test("renders plastic fragmentation without implying biodegradation",()=>{for(const term of ["spark-plastic-persistence-svg","pp-bottle","pp-stress","pp-fragments","pp-microplastics","fragmentation ≠ biodegradation","weathering weakens the material"]) expect(explorer).toContain(term);expect(css).toContain(".spark-plastic-persistence-svg");expect(css).toContain(".pp-microplastics circle");});
 test("covers sea-turtle harm",()=>{expect(migration).toContain("plastic bags can resemble jellyfish");expect(migration).toContain("block or injure the digestive tract");expect(explorer).toContain("Sea turtles");expect(explorer).toContain("pl-turtle-shell");expect(explorer).toContain("pl-bag");expect(explorer).toContain("jellyfish prey");});
 test("renders microplastic food-web transfer and working use controls",()=>{expect(explorer).toContain("pl-micro");expect(explorer).toContain("pl-plankton");expect(explorer).toContain("pl-fish");expect(explorer).toContain("Object.entries(uses)");expect(css).toContain(".spark-microplastic-chain svg");});
 ...
 test("covers recycling and waste reduction",()=>{expect(migration).toContain("Recycling does not make plastic biodegradable");expect(migration).toContain("Refuse unnecessary single-use items");expect(explorer).toContain("Waste reduction");expect(explorer).toContain("Object.entries(options)");expect(explorer).toContain("const data=options[mode]");});
