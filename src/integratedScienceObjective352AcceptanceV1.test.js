...
describe("Integrated Science Objective 3.5.2 acceptance audit",()=>{
 ...
 test("renders acid reaction apparatus and hydrogen evolution",()=>{expect(explorer).toContain("spark-acid-svg");expect(explorer).toContain("mr-test-tube-clip");expect(explorer).toContain("dilute hydrochloric acid");expect(explorer).toContain("hydrogen gas bubbles");expect(explorer).toContain("mr-bubble");expect(css).toContain(".spark-acid-apparatus");});
 ...
 test("covers displacement",()=>{expect(migration).toContain("Iron is more reactive than copper");expect(migration).toContain("copper(II) sulfate");expect(explorer).toContain("copper metal deposited");});
 test("renders displacement apparatus and copper deposition",()=>{expect(explorer).toContain("spark-displacement-svg");expect(explorer).toContain("mr-beaker");expect(explorer).toContain("copper(II) sulfate solution");expect(explorer).toContain("mr-copper-deposit");expect(explorer).toContain("Cu²⁺ ions gain electrons and form Cu");expect(css).toContain(".spark-displacement-apparatus");});
