...
describe("Integrated Science Objective 3.7.1 acceptance audit",()=>{
 ...
 test("renders dust effects through leaf anatomy, light and stomatal gas exchange",()=>{for(const term of ["spark-leaf-dust-svg","ld-cuticle","ld-palisade","ld-spongy","ld-guard-cell","ld-stoma","less light reaches photosynthetic tissue","dust can partly cover stomata","CO₂ enters"]) expect(explorer).toContain(term);expect(css).toContain(".spark-leaf-dust-svg");expect(css).toContain(".ld-light.reduced");expect(css).toContain(".ld-dust-layer circle");});
