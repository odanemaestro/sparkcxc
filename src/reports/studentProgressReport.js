...
function addFooter(page, pageNumber, totalPages) {
  ...
  page.text("Generated by SPARK - CSEC Mathematics", 42, 25, 8.5, false, "#6B7C93");
...
export function generateStudentProgressPdfBytes({ studentName, report, parentName = "", subject = "CSEC Mathematics" }) {
  ...
  const activityPairs = [
    ...
    ["Questions attempted", activity.questionsAttempted || 0],
    ["Question accuracy", percentMetric(activity.questionAccuracy, Boolean(activity.hasQuestionAccuracy))],
    ["Full exams completed", activity.examsCompleted || 0],
    ["Exam average", percentMetric(activity.examAverage, Boolean(activity.hasExamAverage))],
    ["Tutor sessions", activity.tutorSessions || 0],
    ["Flashcards reviewed", activity.flashcardsReviewed || 0],
    ["Study Circle", report.studyCircle?.active ? (Number(report.studyCircle?.group_size || 0) > 0 ? `Active - ${Number(report.studyCircle.group_size)} students` : "Active") : "Not active"],
  ];
  ...
  let y2 = 742;
  y2 = sectionTitle(page2, "Recent exam results", y2);
  if (report.exams?.length) {
    ...
    page2.text("No full-paper results in this reporting period.", 48, y2, 10, false, "#6B7C93");
    y2 -= 24;
  }
