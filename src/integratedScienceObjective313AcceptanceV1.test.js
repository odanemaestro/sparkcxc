...
describe("Integrated Science Objective 3.1.3 acceptance audit",()=>{
 ...
 test("renders all eight planets in one ordered scientific SVG",()=>{
  ["spark-solar-order-svg","ss-order-sun","ss-order-planet","INNER ROCKY PLANETS","OUTER GIANT PLANETS","increasing distance from the Sun","planet sizes and distances are not to scale"].forEach(term=>expect(explorer).toContain(term));
  expect(explorer).toContain("ORDER_POSITIONS");
  expect(css).toContain(".spark-solar-order-svg");
  expect(css).toContain(".ss-order-planet.jupiter");
  expect(css).toContain(".ss-order-rings");
 });
 ...
 test("places the Sun at one focus rather than the ellipse centre",()=>{
  ["spark-solar-orbit-svg","focusOffset=Math.sqrt","sunX=centerX-focusOffset","sse-ellipse-centre","sse-focus-marker","sse-focus-guide","Sun at one focus","rather than at the geometric centre"].forEach(term=>expect(explorer).toContain(term));
  expect(css).toContain(".sse-focus-marker");
  expect(css).toContain(".sse-major-axis");
 });
