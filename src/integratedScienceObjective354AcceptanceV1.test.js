...
describe("Integrated Science Objective 3.5.4 acceptance audit",()=>{
 ...
 test("renders alloy lattice geometry",()=>{
  ["spark-alloy-lattice-svg","al-atom different","al-slip","al-blocked-slip","different-sized atoms distort the lattice and hinder layer movement","pure metal: regular metallic lattice"].forEach(term=>expect(explorer).toContain(term));
  expect(css).toContain(".spark-alloy-lattice-svg");
  expect(css).toContain(".al-stop");
 });
