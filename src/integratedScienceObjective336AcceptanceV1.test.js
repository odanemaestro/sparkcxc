...
describe("Integrated Science Objective 3.3.6 acceptance audit",()=>{
 ...
 test("covers treatment plant stages",()=>{expect(migration).toContain("Sedimentation");expect(migration).toContain("Alum");expect(migration).toContain("Filtration");expect(migration).toContain("chlorine");expect(explorer).toContain("Treatment plant");expect(explorer).toContain("Typical municipal water-treatment sequence");expect(explorer).toContain("alum + mixing");expect(explorer).toContain("flocs settle as sludge");expect(explorer).toContain("sand");expect(explorer).toContain("gravel");expect(explorer).toContain("controlled chlorine dose");expect(explorer).toContain("const stages={");expect(explorer).toContain("Object.entries(stages)");expect(explorer).toContain("aria-pressed={stage===key}");});
 ...
 test("covers desalination and reverse osmosis",()=>{expect(migration).toContain("Desalination is the removal");expect(migration).toContain("Reverse osmosis applies pressure");expect(explorer).toContain("Reverse osmosis");expect(explorer).toContain("spark-ro-svg");expect(explorer).toContain("semi-permeable membrane");expect(explorer).toContain("salt ions rejected");expect(explorer).toContain("fresh-water permeate");expect(explorer).toContain("concentrated brine leaves");});
